...
app.post('/api/submit-visto-negado', async (req, res) => {
  // Garantia adicional: tenta obter dados do body ou do raw (fallback)
  let data = req.body;
  if (!data || Object.keys(data).length === 0) {
    // Se veio como text/plain ou raw, tenta fazer parse manual
    try {
      if (req.rawBody) data = JSON.parse(req.rawBody);
    } catch(e) {}
  }
  ...
  // Validação básica
  if (!data || !data.nome) {
    console.error('❌ Body inválido ou sem nome');
    return res.status(400).json({ error: 'Dados incompletos' });
  }
  
  // Responde imediatamente para não travar o WordPress
  res.status(200).json({ success: true });
  ...
  try {
    ...
    await resend.emails.send({
      ...
      html: `<strong>Avaliação de visto negado recebida.</strong><br>
             ...
             <p>PDF com todas as respostas em anexo.</p>`,
