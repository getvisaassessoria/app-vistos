...
app.post('/api/submit-visto-negado', async (req, res) => {
  console.log('🔔 ROTA VISTO NEGADO ACIONADA');
  console.log('Content-Type:', req.headers['content-type']);
  console.log('Body recebido (typeof):', typeof req.body);
  console.log('Body keys:', req.body ? Object.keys(req.body) : 'vazio');
  
  let data = req.body;
  if (!data || Object.keys(data).length === 0) {
    console.log('⚠️ Body vazio, tentando ler raw...');
    let rawBody = '';
    req.on('data', chunk => rawBody += chunk);
    req.on('end', () => {
      try {
        data = JSON.parse(rawBody);
        console.log('Raw parseado com sucesso:', data);
      } catch(e) {
        console.error('Falha ao parsear raw body:', e.message);
      }
    });
  }
  
  // Aguarda o parsing do raw caso necessário (simplificando, vamos usar um pequeno timeout)
  if (!data || !data.nome) {
    // Tenta novamente com um delay curto (para o caso de leitura raw)
    await new Promise(r => setTimeout(r, 100));
    if (req.rawBody) {
      try { data = JSON.parse(req.rawBody); } catch(e) {}
    }
  }
  ...
  if (!data || !data.nome) {
    console.error('❌ Body inválido – sem nome');
  ...
  try {
    ...
    console.log(`Processando PDF para ${nome}`);
    
    // Geração do PDF com try-catch específico
    let pdfBuffer;
    try {
      pdfBuffer = await new Promise((resolve, reject) => {
        ...
        doc.text(`Nome completo: ${data['nome'] || 'Não informado'}`);
