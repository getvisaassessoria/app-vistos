...
// ==================== CRIAR COMPROMISSO (COM VÍNCULO AUTOMÁTICO AO CLIENTE) ====================
app.post('/api/compromissos', async (req, res) => {
  ...
  try {
    ...
    if (!cliente) {
      ...
      // Atualiza telefone/nome se veio diferente (opcional)
      const updates = {};
    ...
    // 3. Criar compromisso vinculado ao cliente
    const { data: compromisso, error: compError } = await supabase
      .from('compromissos')
      .insert({
        cliente_id: cliente.id,
        cliente: `${cliente.nome_completo} (+${cliente.telefone || ''})`, // campo antigo (compatibilidade)
        atividade: atividade,
        data: data,
        hora: hora,
        local: local || null,
