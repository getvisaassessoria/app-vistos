...
app.use((req, res, next) => {
    ...
    for (const bot of USER_AGENTS_BLOQUEADOS) {
...
function isEmailClienteValido(email, nomeCliente) {
    ...
    for (const suspeito of NOMES_SUSPEITOS) {
        if (nomeLower.includes(suspeito) || emailLower.includes(suspeito)) {
            console.log(`🚨 Padrão suspeito detectado`);
            return false;
        }
    }
    ...
    if (!temVogal && nomeSemEspacos.length > 5) {
        console.log(`🚨 Nome sem vogais: ${nomeCliente}`);
        return false;
    }
...
async function salvarClienteESolicitacao(emailCliente, nomeCliente, telefoneCliente, tipo, dados) {
    try {
        console.log('💾 Salvando no Supabase:', { emailCliente, nomeCliente, telefoneCliente });
        ...
        console.log(`✅ ${tipo} salvo com sucesso! ID: ${solicitacao.id}`);
...
async function enviarEmail(destinatario, assunto, html, attachments = []) {
    try {
...
// ==================== FUNÇÃO PARA GERAR PDF GENÉRICA ====================
async function gerarPDF(titulo, campos, data) {
    return new Promise((resolve, reject) => {
        ...
        doc.fillColor('#003366').fontSize(22).text(titulo, { align: 'center' });
        ...
        for (const field of campos) {
            ...
            if (value && value !== '') {
                if (field.name.includes('data') || field.name.includes('nasc') || field.name.includes('chegada') || field.name.includes('saida')) {
...
app.post('/api/submit-ds160', async (req, res) => {
    ...
    (async () => {
        try {
            ...
            await salvarClienteESolicitacao(emailCliente, nome, telefoneCliente, 'ds160', data);
            ...
            await enviarEmail(
                ...
                `🇺🇸 DS-160: ${nome}`,
                `<strong>Formulário DS-160 recebido.</strong><br><p><strong>Cliente:</strong> ${nome}</p><p>PDF em anexo (${pdfBuffer.length} bytes).</p>`,
                [{ filename: `DS160_${nome.replace(/[^a-z0-9]/gi, '_')}.pdf`, content: pdfBuffer.toString('base64') }]
            );

            if (emailCliente && emailCliente.trim() !== '' && isEmailClienteValido(emailCliente, nome)) {
                await enviarEmail(
                    emailCliente,
                    `Seu formulário DS-160 foi recebido - ${nome}`,
                    `<strong>Olá ${nome},</strong><br><p>Recebemos seu formulário. Segue em anexo uma cópia.</p><p>Em breve nossa equipe entrará em contato.</p>`,
                    [{ filename: `DS160_${nome.replace(/[^a-z0-9]/gi, '_')}.pdf`, content: pdfBuffer.toString('base64') }]
                );
            }
        } catch (err) {
...
app.post('/api/submit-passaporte', async (req, res) => {
    ...
    (async () => {
        try {
            ...
            await salvarClienteESolicitacao(emailCliente, nome, telefoneCliente, 'passaporte', data);
            ...
            const pdfBuffer = await gerarPDF('SOLICITAÇÃO DE PASSAPORTE', campos, data);
            console.log(`📄 PDF gerado para passaporte de ${nome}, tamanho: ${pdfBuffer.length} bytes`);

            await enviarEmail(
                'getvisa.assessoria@gmail.com',
                `📘 Passaporte: ${nome}`,
                `<strong>Solicitação de passaporte recebida.</strong><br><p><strong>Cliente:</strong> ${nome}</p><p>E-mail: ${emailCliente || 'não informado'}</p><p>Telefone: ${telefoneCliente || 'não informado'}</p><p>PDF em anexo (${pdfBuffer.length} bytes).</p>`,
                [{ filename: `Passaporte_${nome.replace(/[^a-z0-9]/gi, '_')}.pdf`, content: pdfBuffer.toString('base64') }]
            );

            if (emailCliente && emailCliente.trim() !== '' && isEmailClienteValido(emailCliente, nome)) {
                await enviarEmail(
                    emailCliente,
                    `Sua solicitação de passaporte foi recebida - ${nome}`,
                    `<strong>Olá ${nome},</strong><br><p>Recebemos sua solicitação de passaporte. Em breve nossa equipe entrará em contato.</p><p>Segue em anexo uma cópia do seu pré-cadastro.</p><br><p>Atenciosamente,<br>Equipe GetVisa</p>`,
                    [{ filename: `Passaporte_${nome.replace(/[^a-z0-9]/gi, '_')}.pdf`, content: pdfBuffer.toString('base64') }]
                );
            }
        } catch (err) {
...
app.post('/api/submit-visto-negado', async (req, res) => {
    ...
    (async () => {
        try {
            ...
            await salvarClienteESolicitacao(emailCliente, nome, telefoneCliente, 'visto_negado', data);
            ...
            await enviarEmail(
                'getvisa.assessoria@gmail.com',
                `⚠️ Visto Negado: ${nome}`,
                `<strong>Avaliação de visto negado recebida.</strong><br>
                ...
                [{ filename: `Visto_Negado_${nome.replace(/[^a-z0-9]/gi, '_')}.pdf`, content: pdfBuffer.toString('base64') }]
            );
            ...
            if (emailCliente && emailCliente.trim() !== '' && isEmailClienteValido(emailCliente, nome)) {
                ...
                await enviarEmail(
                    emailCliente,
                    `Resultado da sua avaliação de visto negado - ${nome}`,
...
app.post('/api/submit-australia', async (req, res) => {
    ...
    (async () => {
        try {
            ...
            await salvarClienteESolicitacao(emailCliente, nome, telefoneCliente, 'australia', data);
            ...
            await enviarEmail(
                'getvisa.assessoria@gmail.com',
                `🇦🇺 Visto Australiano: ${nome}`,
                `<strong>Solicitação de Visto Australiano recebida.</strong><br>
                 <p><strong>Cliente:</strong> ${nome}</p>
                 <p><strong>E-mail:</strong> ${emailCliente || 'não informado'}</p>
                 <p><strong>Telefone:</strong> ${telefoneCliente || 'não informado'}</p>
                 ...
                 <p>PDF em anexo.</p>`,
                [{ filename: `Visto_Australiano_${nome.replace(/[^a-z0-9]/gi, '_')}.pdf`, content: pdfBuffer.toString('base64') }]
            );

            if (emailCliente && emailCliente.trim() !== '' && isEmailClienteValido(emailCliente, nome)) {
                await enviarEmail(
                    emailCliente,
                    `Sua solicitação de Visto Australiano foi recebida - ${nome}`,
                    `<strong>Olá ${nome},</strong><br>
