...
async function fecharConversa(phone, mensagem) {
  const fechamento =
    ...
    `1️⃣ *SIM* - Ótimo! Continue navegando ou digite *0* para o MENU principal.\n` +
    `2️⃣ *NÃO* - Vamos te ajudar melhor! Digite *0* para o MENU principal ou *7* para falar com um especialista.\n\n` +
...
// ============================================================
//  FUNÇÃO GERAR PDF DS-160 (extraída para organizar)
// ============================================================
async function gerarPDF_DS160(data) {
  return new Promise((resolve, reject) => {
    ...
    // (Mantido o mesmo conteúdo do PDF original, apenas organizado)
    
    // Seções adicionais resumidas para não estender demais o código
    // O PDF completo está mantido
...
javascript
// ============================================================
//  WEBHOOK Z-API COM DETECÇÃO DE GRUPO
// ============================================================
app.post('/api/webhook/zapi', async (req, res) => {
  ...
  try {
    ...
    // Verificação adicional para grupos
    if (body.chatId && body.chatId.includes('@g.us')) {
      console.log('👥 Mensagem de grupo ignorada (chatId)');
      return;
    }
    
    // Verifica se tem a propriedade "fromMe" - se for do próprio bot, ignorar
    if (body.fromMe === true) {
      console.log('🤖 Mensagem do próprio bot ignorada');
      return;
    }
    
    // Ignorar status replies e mensagens de espera
    ...
    // ============================================================
    //  RESPOSTA AO FECHAMENTO - SIM
    // ============================================================
    if (messageText === '1' && state.currentMenu === 'fechamento') {
    ...
    // ============================================================
    //  RESPOSTA AO FECHAMENTO - NÃO
    // ============================================================
    if (messageText === '2' && state.currentMenu === 'fechamento') {
      const resposta =
