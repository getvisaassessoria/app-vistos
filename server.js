...
app.post('/api/submit-visto-negado', async (req, res) => {
  ...
  try {
    ...
    const pdfBuffer = await new Promise((resolve) => {
      ...
      doc.moveDown(2);
      doc.fontSize(8).fillColor('#999999').text('Documento gerado automaticamente pelo sistema GetVisa.', { 
align: 'center' });
      doc.end();
    });
    ...
    if (emailCliente && emailCliente.trim() !== '') {
      await resend.emails.send({
        ...
        html: `<strong>Olá ${nome},</strong><br>
               <p>Recebemos sua solicitação de análise para reversão de visto negado. Em breve um de nossos 
especialistas entrará em contato.</p>
